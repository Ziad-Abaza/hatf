...
<div class="container-xxl flex-grow-1 container-p-y min-height-600">
    <!-- Header -->
    <h4 class="py-2 text-end"><span class="text-muted fw-light">لوحة التحكم / الخدمات /</span> بانر </h4>

    <!-- Button for adding new banner -->
    <div class="my-4 text-end">
        <a href="{{ route('dashboard.service_banners.create') }}" class="btn btn-primary d-block d-lg-inline-block">
            <i class="ti ti-plus me-1"></i> إنشاء بانر
        </a>
    </div>
            ...
            <table class="table table-hover custom-rtl">
                ...
                <tbody class="text-center">
                    ...
                    <tr class="searchable-item">
                        <td>{{ Str::limit($serviceBaner->title, 15, '...') }}</td>
                        <td>{{ Str::limit($serviceBaner->second_title, 15, '...') }}</td>
                        <td>
                            <a href="#" class="description-preview show-full-description" data-bs-toggle="modal"
                                data-bs-target="#descriptionModal" data-description="{{ $serviceBaner->description }}">
                                {{ Str::limit($serviceBaner->description, 30, '...') }}
                            </a>
                        </td>
                        <td>
                            <img src="{{ $serviceBaner->getFirstMediaUrl('services_baner') ?: asset('default-image.jpg') }}"
                                alt="Banner Image" class="rounded-circle avatar-sm cursor-pointer"
                                data-bs-toggle="modal" data-bs-target="#imageModal"
                                data-image="{{ $serviceBaner->getFirstMediaUrl('services_baner') ?: asset('default-image.jpg') }}" />
                        </td>
                        <td>
                            <div class="dropdown">
        ...
        <div class="card mb-3 searchable-item">
            <div class="card-body custom-rtl">
                <div class="row g-2 mb-3">
                    <div class="col-12">
                        <h6 class="mb-1">العنوان الأول: {{ Str::limit($serviceBaner->title, 15, '...') }}</h6>
                        <h6 class="mb-1">العنوان الثاني: {{ Str::limit($serviceBaner->second_title, 15, '...') }}</h6>
                        <a href="#" class="description-preview show-full-description" data-bs-toggle="modal"
                            data-bs-target="#descriptionModal" data-description="{{ $serviceBaner->description }}">
                            {{ Str::limit($serviceBaner->description, 70, '...') }}
                        </a>
                    </div>
                </div>
                <div class="row g-2 mb-3">
                    <div class="col-12 text-center">
                        <img src="{{ $serviceBaner->getFirstMediaUrl('services_baner') ?: asset('default-image.jpg') }}"
                            alt="Banner Image" class="w-100 h-75 mx-auto cursor-pointer" data-bs-toggle="modal"
                            data-bs-target="#imageModal"
                            data-image="{{ $serviceBaner->getFirstMediaUrl('services_baner') ?: asset('default-image.jpg') }}" />
                        <small class="d-block mt-1">صورة البانر</small>
                    </div>
                </div>
                <div class="d-flex gap-2 justify-content-start pt-2">
                    <a href="{{ route('dashboard.service_banners.edit', $serviceBaner->id) }}"
                        class="btn btn-icon btn-sm btn-outline-primary">
                        <span class="material-symbols-rounded">edit</span>
                    </a>
                    <form action="{{ route('dashboard.service_banners.destroy', $serviceBaner->id) }}" method="POST"
